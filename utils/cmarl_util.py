...
class ContextBuffer:
    def __init__(self, dim=1, max_buffer_size=10000):
        self.dim=dim
        self.max_buffer_size=max_buffer_size
        self.contexts=np.zeros([max_buffer_size, self.dim])
        self.returns=np.zeros([max_buffer_size, 1])
        self.discounted_returns=np.zeros([max_buffer_size, 1])
        self.episode_lengths=np.zeros([max_buffer_size, 1])
        self.size=0
    def update_buffer(self, data):
        contexts, returns, discounted_returns, episode_length= data
        self.contexts[self.size, :]=contexts
        self.returns[self.size, :]=returns
        self.discounted_returns[self.size, :]=discounted_returns
        self.episode_lengths[self.size, :]=episode_length
        self.size+=1
        
    def clear(self):
        self.contexts=np.zeros([self.max_buffer_size, self.dim])
        self.returns=np.zeros([self.max_buffer_size, 1])
        self.discounted_returns=np.zeros([self.max_buffer_size, 1])
        self.episode_lengths=np.zeros([self.max_buffer_size, 1])
        self.size=0
