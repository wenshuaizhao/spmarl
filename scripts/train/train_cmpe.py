...
from envs.mpe.MPE_env import ContextualMPEEnv
from envs.env_wrappers import CMARLSubprocVecEnv, CMARLDummyVecEnv
...
def make_train_env(all_args, teacher=None):
    ...
    if all_args.n_rollout_threads == 1:
        return CMARLDummyVecEnv([get_env_fn(0)])
    else:
        return CMARLSubprocVecEnv([get_env_fn(i) for i in range(all_args.n_rollout_threads)], teacher=teacher)
...
def make_eval_env(all_args, teacher=None):
    ...
    if all_args.n_eval_rollout_threads == 1:
        ...
        return CMARLSubprocVecEnv([get_env_fn(i) for i in range(all_args.n_eval_rollout_threads)], teacher=teacher, eval=True)
...
def parse_args(args, parser):
    ...
    parser.add_argument('--target_mean', type=float,
                        default=8, help="lower context bound")
    ...
    parser.add_argument('--max_num_agents', type=int,
                        default=30, help="number of players")
    ...
    parser.add_argument('--distance_thre', type=float, default=0.12)
