...
from deep_sprl.teachers.spl.alpgmm_dataset import BufferedDataset
from deep_sprl.teachers.abstract_teacher import AbstractTeacher

import warnings
from sklearn.exceptions import ConvergenceWarning

# Ignore all instances of ConvergenceWarning
...
class ALPGMMTeacher(AbstractTeacher):
    def __init__(self, mins=6, maxs=20, target_context=8, fit_rate=250, random_task_ratio=0.2, max_size=None, seed=None, params=dict()):
        self.seed = seed
        if not seed:
        ...
        self.teacher_name='alpgmm'

        # Range of number of Gaussians to try when fitting the GMM
        self.potential_ks = np.arange(2, 5, 1) if "potential_ks" not in params else params["potential_ks"]
        ...
        self.fit_rate = fit_rate if "fit_rate" not in params else params['fit_rate']
        ...
        self.tasks_alps = []
        self.fit_times=0
        self.init_random_sample_flag=True
    ...
    def update_distribution(self, avg_performance, task, reward):
        self.init_random_sample_flag=False
        task=[ele for ele in task]
        reward=[ele for ele in reward]
        self.tasks.extend(task)

        # Compute corresponding ALP
        self.alps.extend([self.alp_computer.compute_alp(task_i, reward_i) for task_i, reward_i in zip(task, reward)])

        # Concatenate task vector with ALP dimension
        self.tasks_alps.extend([np.array([task_i] + [alp_i]).squeeze() for task_i, alp_i in zip(task, self.alps[-len(task):])])

        # if len(self.tasks) >= self.nb_random:  # If initial bootstrapping is done
        if not self.init_random_sample_flag:
            # if (len(self.tasks) / self.fit_rate) >= self.fit_times:  # Time to fit
                # 1 - Retrieve last <fit_rate> (task, reward) pairs
            cur_tasks_alps = np.vstack(self.tasks_alps[-self.fit_rate:])
    ...
    def sample(self, size=1):
        if (len(self.tasks) < self.nb_random and self.init_random_sample_flag) or (np.random.random() < self.random_task_ratio):
            # Random task sampling
            # new_task = self.random_task_generator.sample()
            new_task=np.random.uniform(low=self.mins, high=self.maxs, size=size).reshape((size, 1))
            ...
            new_task = np.random.multivariate_normal(self.gmm.means_[idx], self.gmm.covariances_[idx], size=size)[:,0] 
            new_task = np.clip(new_task, self.mins, self.maxs).astype(np.float32).reshape((size, 1))

        return new_task

    def _rescale_context(self, context):
        b = 0.5 * (self.maxs + self.mins)
        m = 0.5 * (self.maxs - self.mins)
        return m * context + b

    def _inv_rescale_context(self, internal_context):
        b = 0.5 * (self.maxs + self.mins)
        m = 0.5 * (self.maxs - self.mins)
        return (internal_context - b) / m
