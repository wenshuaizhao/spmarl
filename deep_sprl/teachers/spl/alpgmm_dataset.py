...
class Databag(object):
    ...
    def nn(self, x, k=1, radius=np.inf, eps=0.0, p=2):
        ...
        """
        assert len(x) == self.dim, 'dimension of input {} does not match expected dimension {}.'.format(len(x),
                                                                                                        self.dim)
        k_x = min(k, self.size)
...
class Dataset(object):
    ...
    def get_dims(self, index, dims_x=None, dims_y=None, dims=None):
        if dims is None:
            return np.hstack(
                (np.array(self.data[0][index])[dims_x], np.array(self.data[1][index])[np.array(dims_y) - self.dim_x]))
        else:
    ...
    def nn_dims(self, x, y, dims_x, dims_y, k=1, radius=np.inf, eps=0.0, p=2):
        ...
        if len(dims_x) == 0:
            kdtree = scipy.spatial.cKDTree(
                [np.array(data_y)[np.array(dims_y) - self.dim_x] for data_y in self.data[DATA_Y]])
        elif len(dims_y) == 0:
            kdtree = scipy.spatial.cKDTree([np.array(data_x)[dims_x] for data_x in self.data[DATA_X]])
        else:
            kdtree = scipy.spatial.cKDTree(
                [np.hstack((np.array(data_x)[dims_x], np.array(data_y)[np.array(dims_y) - self.dim_x])) for
                 data_x, data_y in zip(self.data[DATA_X], self.data[DATA_Y])])
    ...
    def _build_tree(self, side):
        ...
        if not self.nn_ready[side]:
            self.kdtree[side] = scipy.spatial.cKDTree(self.data[side], compact_nodes=False,
                                                      balanced_tree=False)  # Those options are required with scipy >= 0.16
...
class BufferedDataset(Dataset):
    ...
    # TODO check if buffer is considered for NN prediction - if not fix this!
