...
class AbstractSPMARLTeacher:
    ...
    def _compute_expected_performance(self, dist, cons_t, old_c_log_prob_t, c_val_t):
        ...
        # print('con_ratio_t is:',con_ratio_t)
        # print('c_val_t is:', c_val_t)
        return torch.mean(con_ratio_t * c_val_t)


class SPMARLTeacher(AbstractTeacher, AbstractSPMARLTeacher):
    ...
    def __init__(self, target_mean, target_variance, initial_mean, initial_variance, context_bounds, perf_lb,
        ...
        super(SPMARLTeacher, self).__init__(initial_mean, flat_init_chol, target_mean, flat_target_chol, max_kl)

    def update_distribution(self, avg_performance, contexts, values, contexts_for_loss, loss):
        # TODO: \
        ...
        contexts_for_loss_t=to_float_tensor(contexts_for_loss, use_cuda=False, dtype=torch.float64)
        old_c_for_loss_log_prob_t = old_context_dist.log_pdf_t(contexts_for_loss_t).detach()
        ...
        try:
            ...
            # even once
                ...
                print("Optimizing Losses")
                constraints = [kl_constraint]

                # Define the objective plus Jacobian
                def objective(x):
                    dist = GaussianTorchDistribution.from_weights(self.context_dim, x, dtype=torch.float64)
                    perf = self._compute_expected_performance(dist, contexts_for_loss_t, old_c_for_loss_log_prob_t, c_loss_t)
                    mu_grad, chol_flat_grad = torch.autograd.grad(perf, dist.parameters())
