...
class Sensor(ABC):
    def __init__(self, world: core.World):
        super().__init__()
        self._world = world
        self._agent: Union[core.Agent, None] = None

    @property
    def agent(self) -> Union[core.Agent, None]:
        return self._agent

    @agent.setter
    def agent(self, agent: core.Agent):
        self._agent = agent
...
class Lidar(Sensor):
    def __init__(
        ...
        n_rays: int = 8,
        max_range: float = 1.0,
        entity_filter: Callable[[core.Entity], bool] = lambda _: True,
    ...
    def entity_filter(
        self, entity_filter: Callable[[core.Entity], bool]
    ):
        self._entity_filter = entity_filter
