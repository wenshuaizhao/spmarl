...
class Scenario(BaseScenario):
    def make_world(self, batch_dim: int, device: torch.device, **kwargs):
        ...
        self.sparse_reward = kwargs.pop("sparse_reward", False)
        self.distance_threshold = kwargs.pop("distance_threshold", 5)
        ...
        # Make world
        world = World(batch_dim, device, gravity=(0.0, -0.05), y_semidim=1) # y_semidim is originally set 1
        # Add agents
        ...
        line_pos = torch.cat(
            [
                ...
                torch.full(
                    ...
                    device=self.world.device,
                    dtype=torch.float32,
                ),  # Y-coordinate (fixed near the bottom)
            ],
            dim=1,
        )

        # Goal position - vertically aligned with the line
        goal_pos = torch.cat(
            [
                line_pos[:, 0:1],  # X-coordinate same as the line
                torch.zeros(
                    (1, 1) if env_index is not None else (self.world.batch_dim, 1),
                    device=self.world.device,
                    dtype=torch.float32,
                    ...
                    0.0,
                    self.world.y_semidim,
                ),  # Y-coordinate (random height)
            ],
            dim=1,
        )

        # Package position - vertically aligned and placed in the middle of the line
        package_rel_pos = torch.cat(
            [
                torch.zeros_like(line_pos[:, 0:1]),  # X-coordinate same as the line
                torch.full(
                    (1, 1) if env_index is not None else (self.world.batch_dim, 1),
                    self.package.shape.radius,
                    device=self.world.device,
                    dtype=torch.float32,
                ),  # Y-coordinate slightly above the line
            ],
            dim=1,
        )
    ...
    def reward(self, agent: Agent):
        ...
        if self.sparse_reward:
            if is_first:
                ...
                # self.ground_rew[self.on_the_ground] = self.fall_reward
                global_shaping = self.package_dist * self.shaping_factor
                if global_shaping < self.distance_threshold:
                    # self.pos_rew = torch.tensor([10.0], device=self.world.device)
                    self.pos_rew = self.global_shaping - global_shaping
                else:
                    self.pos_rew = torch.tensor([0.0], device=self.world.device)
