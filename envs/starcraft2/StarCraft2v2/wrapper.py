...
class StarCraftCapabilityEnvWrapper(MultiAgentEnv):
    ...
    def update_context(self, context=None):
        if context is not None:
            # self.env.close()
            self.kwargs["capability_config"]["n_units"] = int(context)
            self.distribution_config = self.kwargs["capability_config"]
            self.env_key_to_distribution_map = {}
            self._parse_distribution_config()
            self.env.n_agents = int(context)
            # self.env = StarCraft2Env(**self.kwargs)
        else:
