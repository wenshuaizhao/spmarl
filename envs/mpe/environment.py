...
class MultiAgentEnv(gym.Env):
    ...
    def __init__(self, args, world, reset_callback=None, reward_callback=None,
                 observation_callback=None, share_observation_callback=None, info_callback=None,
        ...
        self.share_observation_callback=share_observation_callback
        ...
        self.share_obs_dim=len(self.share_observation_callback(self.world))
        
        self.obs_dim=obs_dim
        self.full_obs=np.zeros([self.args.max_num_agents, self.obs_dim])
        self.full_share_obs=np.zeros([self.args.max_num_agents, self.share_obs_dim])
        ...
        self.share_observation_space = [spaces.Box(
            low=-np.inf, high=+np.inf, shape=(self.share_obs_dim,), dtype=np.float32) for _ in range(self.n)]
    ...
    def step(self, action_n):
        self.current_step += 1
        obs_n = self.full_obs.copy()
        share_obs_n=self.full_share_obs.copy()
        full_reward_n = self.full_rewards.copy()
        reward_n=[]
        done_n = self.full_dones.copy()
        info_n = self.full_infos.copy()
        self.agents = self.world.policy_agents
        # set action for each agent
        for i, agent in enumerate(self.agents):
            self._set_action(action_n[i], agent, self.action_space[i])
        ...
        if self.shared_reward:
            reward_n = [[reward]] * self.n
        for i, agent in enumerate(self.agents): full_reward_n[i]=reward_n[i]
        for i, agent in enumerate(self.agents): share_obs_n[i]=self.share_observation_callback(self.world)
        
        if self.post_step_callback is not None:
            self.post_step_callback(self.world)
        info_n[:self.n,:]=1.

        return obs_n, share_obs_n, full_reward_n, done_n, info_n
